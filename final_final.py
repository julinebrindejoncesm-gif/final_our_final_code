...
PLAYER_IMAGE_FILENAME = "method.png"  # put this file in the SAME folder as this .py file
...
# Enemy settings (now map-driven: any level with 'E' has enemies)
ENEMY_MOVE_EVERY_TICKS = 1  # 1 = move every game tick, 2 = every other tick (slower enemies)

# Bonus settings (now map-driven: any level with 'B' has bonuses)
BONUS_POINTS = 200
...
def load_player_image(cell_size: int):
    """
    Loads method.png from the same folder as this script.
    IMPORTANT: move method.png off Desktop into the project folder (same folder as this .py),
    or update PLAYER_IMAGE_FILENAME to an absolute path.
    """
    script_dir = os.path.dirname(os.path.abspath(__file__))
    img_path = os.path.join(script_dir, PLAYER_IMAGE_FILENAME)

    if not os.path.exists(img_path):
        raise FileNotFoundError(
            f"Couldn't find '{PLAYER_IMAGE_FILENAME}' next to your .py file.\n"
            f"Move method.png into the same folder as this script, or change PLAYER_IMAGE_FILENAME.\n"
...
def rotated_player_image(img, direction):
    # direction is (dx, dy)
    if direction == (0, -1):   # up
...
def main():
    ...
    try:
        player_img_base = load_player_image(CELL_SIZE)
    ...
    def load_level(level_index: int):
        ...
        # --- Key/lock system (still Level 4 only) ---
        has_key = False
        key_cells = []
        collected_keys = set()
        key_popup_ticks = 0
    ...
    while True:
        ...
        if state == "PLAYING":
            ...
            if blocked:
                ...
                # ---- Bonus pickup (map-driven: any level with B) ----
                if bonuses_active:
            ...
            # ---- Move enemies (map-driven: any level with E) ----
        ...
        if state == "MENU":
            ...
            if menu_error:
                draw_text(screen, "Level error:", 22, RED, (w // 2, h // 2 + 220))
                draw_text(screen, menu_error, 18, RED, (w // 2, h // 2 + 250))
        ...
        # Player sprite (replaces the green square head)
        head_px = snake[0]
        player_img = rotated_player_image(player_img_base, direction)
        screen.blit(player_img, head_px)
